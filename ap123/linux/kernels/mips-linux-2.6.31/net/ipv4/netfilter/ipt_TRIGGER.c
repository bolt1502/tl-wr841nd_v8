...
static unsigned int
trigger_dnat(struct sk_buff *skb, const struct xt_target_param *param)
{
	int ret;
	struct ipt_trigger *found;
	const struct iphdr *iph = ip_hdr(skb);
	...
	DEBUGP("%s:  found->srcip  %x;  found->dstip  %x;param->hooknum %d; IP_NAT_MANIP_SRC %d \n",
		__FUNCTION__, found->srcip, found->dstip, param->hooknum, IP_NAT_MANIP_SRC);
...
trigger_check(const struct xt_tgchk_param *param)
{
	...
	if ( param->target->targetsize != IPT_ALIGN(sizeof(*info))) {
