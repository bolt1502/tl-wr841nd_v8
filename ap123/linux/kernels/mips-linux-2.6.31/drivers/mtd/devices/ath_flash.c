...
unsigned int ath_spi_read_cmd(u32 cmd)
{
	u32 ret = 0;
	
	ath_reg_wr_nf(ATH_SPI_FS, 1);
	ath_reg_wr_nf(ATH_SPI_WRITE, ATH_SPI_CS_DIS);	/* Chip Select */
	
   	ath_spi_bit_banger(cmd);
	ret = ath_spi_raw_input_u32();
...
void ath_spi_write_cmd(u32 cmd, u8 val)
{
	ath_spi_write_enable();
...
static void ath_flash_unlock(void)
{
	u32 flashid;
	u32 stat;

	flashid = ath_spi_read_cmd(0x9f);
	...
	if (flashid >> 8 == 0x898912)
	{
		...
		stat = ath_spi_read_cmd(ATH_SPI_CMD_RD_STATUS);
		stat = stat >> 24;
		printk("->Oops: old stat 0x%x . \n", stat);
		
		ath_spi_write_cmd(0x01, stat & (~0x1c));
	}
}
