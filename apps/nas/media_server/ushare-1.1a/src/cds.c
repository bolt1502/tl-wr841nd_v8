...
#define CDS_UT_LIST_LOCK()  //rebuild_list_lock()
#define CDS_UT_LIST_UNLOCK() //rebuild_list_unlock()
										 ...
										 char *filter)
{
	...
	/* add by HouXB, 06Feb12 */
	char* mimeClass = mime_get_class(entry->mime_type);
							 ...
							 int count, struct upnp_entry_t *entry, char *filter)
{
	...
	for (; *childs; childs++)
	{
		...
		/* only fetch the requested count number or all entries if count = 0 */
		{
			...
			{
				mimeClass = mime_get_class((*childs)->mime_type);
			...
			else /* item */
			{
				...
				/*set mime type protocol for XBOX.  by HouXB, 25Oct10 */
				char *protocol = NULL;
				if(!strncmp("Xbox", event->request->UserAgent, strlen("Xbox")))
				{
					protocol = mime_get_protocol ((*childs)->mime_type); 
				}
				else
				{
					protocol = dlna_write_protocol_info (DLNA_PROTOCOL_INFO_TYPE_HTTP,
												DLNA_ORG_PLAY_SPEED_NORMAL,
												DLNA_ORG_CONVERSION_NONE,
												DLNA_ORG_OPERATION_RANGE,
				...
				/*end set mime type protocol for XBOX.  by HouXB, 25Oct10 */

				/* special for XBOX debug! by HouXB, 25Oct10 
				printf("\n^^^\nagent is:%s\nprotocol is :%s\nvalue is:%d\ndlnaProfile:%d\nxbox? %d\n^^^\n", 
					event->request->UserAgent, protocol,
					((*childs)->dlna_profile && strncmp("Xbox", event->request->UserAgent, strlen("Xbox"))),
					(*childs)->dlna_profile,strncmp("Xbox", event->request->UserAgent, strlen("Xbox")));
				*/

				/* set mime type class for XBOX. by HouXB, 25Oct10 */
				if(!strncmp("Xbox", event->request->UserAgent, strlen("Xbox")))
				{
					mimeClass = mime_get_class((*childs)->mime_type);
				...
				/* end of set mime type class for XBOX. by HouXB, 25Oct10 */
...
cds_browse (struct action_event_t *event)
{
	...
	/*  
	printf("file: %s,line: %d==: pid %d; ppid %d;--debug by HouXB\n",
	...
	*/
	...
	else
	{
		/*  
		printf("\n***\ngot browse request:\n index: %d\n count:%d\n id: %d\nfilter:%s\nsort_criteria: %d\n***\n", 
			index, count, id,  filter, sort_criteria);
		*/
...
matches_search (char *search_criteria, struct upnp_entry_t *entry)
{
	...
	if (derived_from && entry->mime_type
			&& !strncmp (mimeClass, keyword, strlen (keyword)))
		result = true;
	else if (protocol_contains && strstr (protocol, keyword))
		result = true;
	else if (entry->mime_type && !strcmp (mimeClass, keyword))
		result = true;
	free (mimeClass);
	free (protocol);
			...
			else /* item */
			{
				if (matches_search (search_criteria, *childs))
				{
					...
					char *protocol =
						...
						char* mimeClass = mime_get_class((*childs)->mime_type);
...
#define UPNP_VIDEO_CDS "object.item.videoItem"
#define UPNP_AUDIO_CDS "object.item.audioItem.musicTrack"
#define UPNP_PHOTO_CDS "object.item.imageItem.photo"
...
cds_search (struct action_event_t *event)
{
	...
	search_criteria = upnp_get_string (event->request, SERVICE_CDS_ARG_SEARCH_CRIT);
	filter = upnp_get_string (event->request, SERVICE_CDS_ARG_FILTER);
	sort_criteria = upnp_get_ui4 (event->request, SERVICE_CDS_ARG_SORT_CRIT);
	...
	/*changed for support XBOX audio search. by HouXB, 22Oct10 
	printf("\n***\ngot search request:\n index: %d\n count:%d\n id: %d\nsearch_criteria:%s\nfilter:%s\nsort_criteria: %d\n***\n", 
			index, count, id, search_criteria, filter, sort_criteria);
	*/	
	
	char search_criteria1[] = "(upnp:class = \"" "object.item.audioItem.musicTrack" "\")" ;
	
	if ((!strcmp(search_criteria,  "(upnp:class = \"" UPNP_VIDEO_CDS "\")")) ||
		(!strcmp(search_criteria,  "(upnp:class = \"" UPNP_AUDIO_CDS"\")")) ||
		(!strcmp(search_criteria,  "(upnp:class = \"" UPNP_PHOTO_CDS"\")")) )
	...
	else
	{
		result_count =
		cds_search_directchildren (event, out, index, count, entry,
								     filter, search_criteria1);
	}
	/* end change. by HouXB, 22Oct10 */
...
#undef CDS_UT_LIST_LOCK(x)
#undef CDS_UT_LIST_UNLOCK(x)