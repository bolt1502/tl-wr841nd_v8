...
pthread_mutex_t inRebuildingList;
...
init_upnp (struct ushare_t *ut)
{
	...
	sprintf(modelDescripTmp,"%s : DLNA Media Server", ut->manufature);
	...
	else
	{
		...
		len = strlen (UPNP_DESCRIPTION) + strlen (ut->name)
			+ 2*(strlen (ut->manufature)) + 2*(strlen(ut->manufature_url))
			+ strlen (DEFAULT_SERIAL_NUM) + strlen (ut->model_name) + strlen (ut->udn) + 1;

		description = (char *) malloc (len * sizeof (char));
		memset (description, 0, len);
		
		sprintf (description, UPNP_DESCRIPTION, 
			ut->name, ut->manufature, ut->manufature_url, 
			ut->manufature, ut->model_name, ut->manufature_url, 
			DEFAULT_SERIAL_NUM, ut->udn);
#ifdef HAVE_DLNA
	}
#endif /* HAVE_DLNA */
...
/* #define DEFAULT_SHARE_FOLDER "/tmp/usbdisk/volume1"by HouXB, 29Sep10 */

int initUshare(int argc, char **argv)
{
	int i = 0;	
	...
	/* set parameters from config file. by HouXB, 30Sep10 */
	...
	/* set server name. by HouXB, 29Sep10 */
	if (ut->name)
	{
		free(ut->name);
	}
		ut->name = gUshareInitInfo.serverInfo.servName;
	
	/* set manufatur name. by HouXB, 04Jan12 */
	if (ut->manufature)
	{
		free(ut->manufature);		
	}
	ut->manufature = gUshareInitInfo.serverInfo.manufatureName;
	
	/* set manufatur URL. by HouXB, 04Jan12 */
	if (ut->manufature_url)
	{
		free(ut->manufature_url);		
	}
	ut->manufature_url = gUshareInitInfo.serverInfo.manufatureUrl;
	
	/* set model name. by HouXB, 04Jan12 */
	if (ut->model_name)
	{
		free(ut->model_name);		
	}
	ut->model_name = gUshareInitInfo.serverInfo.modelName;
	
	/* set model URL. by HouXB, 04Jan12 */
	if (ut->model_url)
	{
		free(ut->model_url);
		ut->model_url = gUshareInitInfo.serverInfo.manufatureUrl;
	}

	/* set content list. by HouXB, 29Sep10 */
	if (ut->contentlist)
	{
		free(ut->contentlist);
		ut->contentlist = NULL;
	...
	for (i=0; i<gUshareInitInfo.folderList.validFolderNum; i++)
	{
			/*printf("\n\n--------\nfolder list:%s\n",
					gUshareInitInfo.folderList.folderListInfo[i].contentInfo.fullPath); by HouXB, 30Sep10 */
		
		if (0 != gUshareInitInfo.folderList.folderListInfo[i].contentInfo.fullPath[0])
		{
			ut->contentlist = content_add (ut->contentlist, 
				gUshareInitInfo.folderList.folderListInfo[i].contentInfo.fullPath,
				gUshareInitInfo.folderList.folderListInfo[i].contentInfo.displayName);

			/*  
			printf("add name:%s; path:%s\n",
				gUshareInitInfo.folderList.folderListInfo[i].contentInfo.displayName,
				gUshareInitInfo.folderList.folderListInfo[i].contentInfo.fullPath);*/
		}		
	}
	...
	/* NOT USE ANYMORE by HouXB, 29Sep10 
	if (!ut->contentlist)
	{
		log_error (_("Error: no content directory to be shared.\n"));
		ushare_free (ut);
	...
	*/
	...
	/* NOT USE ANYMORE by HouXB, 30Sep10 
	if (ut->use_telnet)
	{
		...
		ctrl_telnet_register ("kill", ushare_kill, _("Terminates the uShare server"));
	}
	*/
	...
	/* build media list by HouXB, 13Sep10 */
	build_metadata_list (ut);
...
/************************************************************
Function:	daemonize
Description:	将进程设置为daemonize
Input:	chdir:是否修改主目录; closeFd:是否修改标准输入输出文件描述符
Output: 
Return: 0:daemonize 成功; -1:失败
************************************************************/
int daemonize(int chDir, int closeFd) {
	int fd;
	...
	if (closeFd && (fd = open("/dev/null", O_RDWR, 0)) != -1) {
...
/**  OK*/
/*  test media Tree for debug info.by HouXB, 21Oct10 */
void printEntry(struct upnp_entry_t *entryName )
{
	struct upnp_entry_t *entryTmp;
	int ei;
	entryTmp = entryName;

	
	if(entryTmp->child_count == -1)
	{	
		
		printf("id is: %d, parent is %d\n", entryTmp->id, entryTmp->parent->id);
		printf("**file title:%s\n\n", entryTmp->title);
		printf("**file title:%s,fullpath/url:%s++%s\n\n", entryTmp->title,entryTmp->fullpath,entryTmp->url);
		
	}
	else
	{
		if(entryTmp->id != 10000)
		{
			if(entryTmp->parent)
			{
				printf("dir id is: %d, parent is %d\n", entryTmp->id, entryTmp->parent->id);
			}
	...
	for(ei=0; ei<(entryTmp->child_count); ei++)
	{		
		printEntry(entryTmp->childs[ei]);
	}
	return;
}

#define USBDISK_PATH "/tmp/usbdisk/volume1"

inline void rebuild_list_lock()
{
	/* 
	printf("file: %s,line: %d==: pid %d locked in line %d--debug by HouXB\n",
			__FILE__, __LINE__, getpid(), x);
	 */

	pthread_mutex_lock(&inRebuildingList);
}

inline void rebuild_list_unlock()
{
	/* 
	printf("file: %s,line: %d==: pid %d unlocked in line %d--debug by HouXB\n",
			__FILE__, __LINE__, getpid(), x);
	 */
	pthread_mutex_unlock(&inRebuildingList);
}
...
main (int argc, char **argv)
{
	...
	/* for web debug. by HouXB, 18Oct10 */
	int isNormal = 1;

	pthread_mutex_init( &inRebuildingList, NULL );
	...
	if (1 == isNormal)
	{
		serverProcessInit(&sd);

		clientSd = setUshareInitInfo(sd);

		if(clientSd < 0){
			printf("setUshareInitInfo ERROR\n");
			return 0;
		}

		/* wait for usb device was scanned. */
		{
			struct stat st;
			int i;
	...
	else /* just for web debug */
	{
		sprintf(gUshareInitInfo.serverInfo.servName, "%s","TPLINK");
		sprintf(gUshareInitInfo.serverInfo.manufatureName, "%s","TPLINK");
		sprintf(gUshareInitInfo.serverInfo.manufatureUrl, "%s","www.tp-link.com");
		sprintf(gUshareInitInfo.serverInfo.modelName, "%s","TPLINK");
		
		gUshareInitInfo.serverInfo.serverStatus = 1;
		gUshareInitInfo.serverInfo.isAutoScan = 0;
		gUshareInitInfo.folderList.validFolderNum = 1;
		sprintf(gUshareInitInfo.folderList.folderListInfo[0].partition, "%s", "FAT32");
		gUshareInitInfo.folderList.folderListInfo[0].contentInfo.index = 0;
		sprintf(gUshareInitInfo.folderList.folderListInfo[0].contentInfo.fullPath, "%s", "/tmp/usbdisk/volume1/");
	}

	/* printf("begin init ut\n"); */
	initUshare(argc, argv);

	if(ut->xbox360)
	{
		printf("XBOX surpportting was set!\n");
	}

	/* printEntry(ut->root_entry); */

	if (1 == isNormal)
	{
		processHttpdReq(clientSd);
	}
